...
export class Store {
  ...
  disablePrayer(userId, all = false) {
    ...
    if (p.enabled || (all && (Object.values(p.occasions).some(Boolean) || Object.values(p.daily).some(item => item.activatedAt)))) this.setPrayer(userId, { ...p, enabled: false, ...(all ? { occasions: { ...DEFAULT_OCCASIONS }, daily: stopDaily(p.daily) } : {}) });
  }
  ...
  claimPrayer(userId, expected, event, now) {
    const daily = DAILY_REMINDERS.some(([key]) => key === event.key);
    const occasion = ['qada30', 'qada15'].includes(event.key) ? 'qada' : ['fridayPrayer', 'fridayDua'].includes(event.key) ? event.key : null;
    if ((!occasion && !daily && !PRAYERS.some(([key]) => key === event.key)) || !/^\d{4}-\d{2}-\d{2}$/.test(event.day) || !Number.isSafeInteger(event.at)) throw new RangeError('Invalid prayer event');
    return this.transaction(() => {
      const p = this.getPrayer(userId), user = this.getUser(userId);
      const activation = daily ? p.daily[event.key].activatedAt : occasion ? p.occasions[occasion] : p.activatedAt;
      if (JSON.stringify(p) !== JSON.stringify(expected) || !(occasion || daily ? activation > 0 : p.enabled) || user.dmBlocked || user.pausedUntil > now || user.pausedUntil > event.at ||
