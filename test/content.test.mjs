...
test('the shipped religious content matches its source review', () => {
  assert.equal(assertReviewedContent(), 19);
...
test('changes to wording, attribution, narrators or explanations require another review', () => {
  ...
  for (const change of changes) {
    const cards = structuredClone([...DHIKR_CARDS, ...GOOD_DEEDS, ...Object.values(OCCASION_CARDS), ...DAILY_DHIKR, ...Object.values(SEASONAL_CARDS)]);
    assert.equal(assertReviewedContent(cards), 19);
    change(cards[0]);
    assert.throws(() => assertReviewedContent(cards), { code: 'CONTENT_REVIEW_REQUIRED' });
  }
  const cards = [...DHIKR_CARDS, ...GOOD_DEEDS, ...Object.values(OCCASION_CARDS), ...DAILY_DHIKR, ...Object.values(SEASONAL_CARDS)];
